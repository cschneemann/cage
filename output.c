...
update_output_manager_config(struct cg_server *server)
{
	...
	wl_list_for_each (output, &server->outputs, link) {
		...
		/* Outputs that are merely powered off are still part of the output
		 * layout. Report them as enabled, so that clients don't mistake them
		 * for outputs that have been disabled. */
...
/* An output that is not part of the output layout has been disabled, as opposed
 * to an output that is only powered off. */
static inline bool
...
output_set_power(struct cg_output *output, bool on)
{
	...
	/* Disabled outputs are not part of the output layout and thus display
	 * nothing, so leave their power state alone. */
	if (output_is_disabled(output)) {
